...
1.  **FMR 地址生成器 (Address Generator, AG)：**
      ...
      * **功能：** **\<font color="\#00B050"\>（这是您新增的关键部分）\</font\>** AG 接收来自寄存器的浮点坐标 `(x, y)`，**自动在硬件中**计算出：
          * (a) 双线性插值所需的 4 个整数地址。
          * (b) 4 个插值权重 (即 RETA-AD 中的 $\alpha, \beta, \gamma, \lambda$)。
      * **收益：** 此过程**完全不占用** SM 中的 SP/INT 计算单元。

2.  **FMR 内存控制器 (4-Bank SMEM)：**

      * **实现：** 修改 GPGPU-Sim 的 `memory.cc`，在 Shared Memory (SMEM) 模型中划分出一块“可重组”区域。
      * **功能：** 当使用新的`st.rfm.shared`指令写入时，该控制器将根据像素的2D坐标奇偶性，自动将数据“编码”并**交错存入 4 个独立的 SMEM Bank**。
...
3.  **FMR 插值核心 (Bilinear Interpolation Core)：**
      ...
      * **功能：** 该单元在硬件上固化了双线性插值所需的所有数学逻辑（即 4 个乘法器和一个 4-2 压缩器/加法树）。它不占用通用的 SP 单元。
...
我们将引入一条新的 PTX 指令来调用 FMR 硬件，彻底取代“手动”采样代码：
  ...
  * **GPGPU-Sim 中的执行流程：**
    ...
    3.  `FMR_Unit` 执行：
      ...
      b.  [cite\_start]**(1 周期):** 内存控制器根据 4 个整数地址，**并行**向 4 个 SMEM Bank 发出读请求（由于 FMR 的设计，这 4 个请求保证无冲突）。
      c.  **(N 周期):** SMEM 返回 4 个值。
